...
Actor.prototype.freeformSheet = async function(macroId, name) {
	...
	let newSpan = async function(key, value){
		let $span = $(`<span id="${key}" role="textbox" contenteditable="true">
		...
		.focusout(async function(){
			if ($(this).text().trim()=="" || $(this).text().trim() == "NEW TEXT") {
				console.log(`removing span`, name, key)
				await character.unsetFlag('ffs', `${name}.${key}`);
				return $(this).remove();
			}
			$(this).text(TextEditor.enrichHTML(Roll.replaceFormulaData($(this).text().trim(), {...character.toObject(), ...character.getRollData()})))
			await character.setFlag('ffs', [`${name}.${key}`], {text: $(this).text().trim()});
	...
	let d = new Dialog({
		...
		content: `<div class="ffs" style="position: relative;"></div>`,
		buttons: {},
		render: async (html)=>{
			...
			$sheet.before($(`<style>
				#${id} > section.window-content > div.dialog-content > div.ffs {font-family: ${fontFamily}; font-weight: ${fontWeight}; cursor: cell;}
        ...
        #${id} > section.window-content > div.dialog-content > div.ffs > span > input:focus {box-shadow: unset; } 
				#${id} > section.window-content > div.dialog-content > div.ffs > span:focus-visible {outline-color:white; outline-style: outset; outline-offset: 6px;}
